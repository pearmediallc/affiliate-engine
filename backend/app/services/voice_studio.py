...
def _syn_fal_clone(text: str, out_path: str, sample_url: str, ref_text: Optional[str] = None) -> dict:
    ...
    # The per-sentence "echo" (a reference word injected at each sentence start) is PREVENTED upstream
    # by giving f5 a CLEAN, silence-bounded reference + an accurate ref_text (see the ref extraction in
    # recipe_avatar_lipsync). No output patching here — that only masks a bad reference.
    payload = {"gen_text": text[:5000], "ref_audio_url": sample_url,
               "model_type": "F5-TTS", "remove_silence": True, "speed": VOICE_SPEED}
    if ref_text:
        payload["ref_text"] = ref_text[:2000]   # tell f5 EXACTLY what the ref says (else it ASRs it)
