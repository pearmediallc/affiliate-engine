"""AI API core module for Vmake AI SDK."""
...
from vmake_sdk.core.config import USER_AGENT, GID_CACHE_FILE, INVOKE, REGIONS
from vmake_sdk.core.models import TaskResult, TaskStatus
from vmake_sdk.utils.cache import GidCache
from vmake_sdk.auth.signer import Signer, HeaderHost
...
def _progress_log(msg: str) -> None:
    if not PROGRESS_ENABLED:
        return
    print(f"[vmake-ai] {msg}", file=sys.stderr, flush=True)
...
class AiApi:
    ...
    def getFileUrl(self, file):
        ...
        """
        from vmake_sdk.storage.oss import OssUploader, resolve_oss_region, normalize_oss_endpoint
